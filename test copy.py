...
class FishingBot:
    ...
    def wait_for_minigame_window(self, timeout: float = 2.0) -> Optional[GameRegion]:
        """Waits for the fishing minigame window to appear (blue circle detected)."""
...
class BotGUI:
    ...
    def toggle_bot(self):
        ...
        self.region = self.create_center_square_region()
        if not self.region:
            messagebox.showerror("Error", "Could not create fishing region!")
            return
        
        self.add_status(f"Region: {self.region.width}x{self.region.height}")
        
        self.config['human_like_clicking'] = self.human_like_var.get()
        self.config['quick_skip'] = self.quick_skip_var.get()
        self.save_config()
        
        self.bot = FishingBot(self.region, self.config, self.window_manager, bait_counter=self.bait)
